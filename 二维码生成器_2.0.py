...
class qrcodeGUI(QtWidgets.QWidget):
    ...
    def saveQrcode(self):
        ...
        input_text = self.content_edit.text().upper()
        dash_occurrences = input_text.count('-')

        if dash_occurrences == 0:
            spu = input_text
            color = ''
            size = ''
        elif dash_occurrences == 1:
            spu, color = input_text.split('-')
            size = ''
        elif dash_occurrences == 2:
            spu, color, size = input_text.split('-')
        else:
            input_list = input_text.split('-')
            size = input_list[-1]
            color = input_list[-2]
            spu_list = input_list[:-2]
            spu = '-'.join(spu_list)
