...
class qrcodeGUI(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        # 定义组件
        # 	--Label
        self.content_label = QLabel('货号 (max9):')
        ...
        self.grid.addWidget(self.generate_button, 3, 5, 1, 2)
        self.grid.addWidget(self.save_button, 4, 5, 1, 2)
    ...
    def saveQrcode(self):
        ...
        draw.text((15, 70), self.content_edit.text().upper(), (0, 0, 0), font=image_font)
        draw.text((15, 95), self.color_content_edit.text().upper(), (0, 0, 0), font=image_font)
        qr = self.qr_img
        ...
        filename = QFileDialog.getSaveFileName(self, '保存', './' + self.content_edit.text().upper() + '-' + self.color_content_edit.text().upper() + '.png',
                                               '所有文件(*)')
        if filename[0] != '':
